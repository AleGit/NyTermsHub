...
class ParseBasicSyntaxTests: XCTestCase {
    ...
    func testParseHWV105cnf1() {
        let expected = 5.0;
        let ((result,tptpFormulae,tptpIncludes),runtime) = measure {
            parse(path:"HWV105-1".p!)
        }
        XCTAssertTrue(runtime <= expected,"runtime \(runtime.prettyTimeIntervalDescription) ≰ \(expected.prettyTimeIntervalDescription)")
        print("runtime: \(runtime) <≟> \(expected) (expected)")
    ...
    func testParseHWV062fof1() {
        ...
        let expectedCount = 39512;
        XCTAssertEqual(expectedCount, conjunction.nodes!.count)
        ...
        XCTAssertEqual(2, conjunction.nodes![expectedCount-2].nodes!.count)      // 95563
        XCTAssertEqual("p(V6641)|p(V6673)", conjunction.nodes![expectedCount-2].description)
        ...
        XCTAssertEqual(expectedCount, a.nodes!.count)
