...
public extension Term {
    ...
    public var isTupleOfVariables : Bool {
        guard tptpSymbols[self.symbol]?.type == SymbolType.Tuple else { return false }
        guard let terms = self.terms else { return false }
    ...
    var isFunction : Bool {
        ...
        guard let category = tptpSymbols[self.symbol]?.category else { return terms.reduce(true) { $0 && $1.isFunction } }
    ...
    private var isNegativePredicate : Bool {
        guard tptpSymbols[self.symbol]?.type == SymbolType.Negation else { return false }
    ...
    private var isPositivePredicate : Bool {
        guard let type = tptpSymbols[self.symbol]?.type where type == SymbolType.Predicate else { return false }
        guard let terms = self.terms else { return false }
    ...
    private var isInequation : Bool {
        guard let type = tptpSymbols[self.symbol]?.type else { return false }
        guard let terms = self.terms else { return false }
    ...
    public var isEquation : Bool {
        guard let type = tptpSymbols[self.symbol]?.type where type == SymbolType.Equation else { return false }
    ...
    private var isFormula : Bool {
        ...
        guard let quadruple = tptpSymbols[self.symbol]
    ...
    public var isDisjunctionOfLiterals : Bool {
        ...
        guard tptpSymbols[self.symbol]?.type == SymbolType.Disjunction else { return false }
