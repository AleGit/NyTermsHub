...
extension Node {
    ...
    private var isInequation : Bool {
        guard let type = self.symbol.type else { return false }    // an undefined node type cannot be an inequation
    ...
    private var isDisjunctionOfLiterals : Bool {
        ...
        guard let type = self.symbol.type where type == SymbolType.Disjunction else { return false }
    ...
    var isHornClause : Bool {
        ...
        guard let type = self.symbol.type where type == SymbolType.Disjunction else {
